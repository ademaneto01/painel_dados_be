...
export default function Alunados(): JSX.Element {
  const [error, setError] = useState(false);
  const [msgError, setMsgError] = useState('');
  const { idEntidadeEscolar, setPage, setGlobalAnoRef } = useGlobalContext();
  ...
  const handleNavigate = () => {
    setPage(PageEnumContratos.cadastroAlunado);
    setGlobalAnoRef(formAnoRef);
  };
  const handleNavigateEdit = () => {
    setPage(PageEnumContratos.editarAlunado);
    setGlobalAnoRef(formAnoRef);
  };
